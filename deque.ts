abstract class ParentQueue<T> {
    ...
    abstract remove_front(): void {
        if (this.size() > 0) {
            this.remove_front_status = REMOVE_FRONT_OK;
            return this._queue.remove[0];
        } else {
            this.remove_front_status = REMOVE_FRONT_ERR;
        }
    }
    ...
    abstract get_front(): T { // получить элемент из головы очереди;
        if (this.size() > 0) {
            this.get_front_status = GET_FRONT_OK;
            return this._queue[0];
        } else {
            this.get_front_status = GET_FRONT_ERR;
        }
    };
...
abstract class Deque<T> extends ParentQueue<T> {
    ...
    abstract remove_tail(): void {
        if (this.size() > 0) {
            this.remove_tail_status = REMOVE_TAIL_OK;
            return this._queue.remove[this.size() - 1];
        } else {
            this.remove_tail_status = REMOVE_TAIL_ERR;
        }
    }
    ...
    abstract get_tail(): T { // получить элемент из хвоста очереди;
        if (this.size() > 0) {
            this.get_tail_status = GET_TAIL_OK;
            return this._queue[this.size() - 1];
        } else {
            this.get_tail_status = GET_TAIL_ERR;
        }
    }
